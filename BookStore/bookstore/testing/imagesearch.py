
import time

from selenium.webdriver.common.keys import Keys
from selenium.webdriver.chrome.service import Service
from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.support.ui import Select


service = Service(executable_path=r"C:\Users\jesna\OneDrive\Documents\Bookstore-C\BookStore\bookstore\chromedriver.exe")
driver = webdriver.Chrome(service=service)
driver.maximize_window()


driver.get("http://127.0.0.1:8000/login/login")

# Locate the username and password fields and input the credentials
username_field = driver.find_element(By.NAME, "email")
username_field.send_keys("jesnamol1112@gmail.com")

password_field = driver.find_element(By.NAME, "password")
password_field.send_keys("Jesna@123")

# Submit the login form
password_field.send_keys(Keys.RETURN)
driver.get("http://127.0.0.1:8000/book_search/")
# Fill in the form fields
book_cover = driver.find_element(By.NAME, "book_cover")
book_cover.send_keys(r"C:\Users\jesna\OneDrive\Documents\Bookstore-C\BookStore\bookstore\media\pics\app.jpg")

# Submit the form
submit_button = driver.find_element(By.XPATH,'/html/body/div[3]/div/form/button')
submit_button.click()

driver.get("http://127.0.0.1:8000/book_search/")


# Wait for the page to load and check for the presence of the dashboard element

dashboard_element = driver.find_element(By.XPATH, "//h1[contains(text(), 'Book Search')]")
if dashboard_element:
    print("Test Passed")
else:
    print("Test failed.")
